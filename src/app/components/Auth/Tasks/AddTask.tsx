...
export default function AddTask({ AddTaskActions }: any) {
    const { Close, BoardId, getBoardData } = AddTaskActions
    ...
    const onSubmit: SubmitHandler<AddTaskForm> = async (data) => {
        try {
            await axios.post(`/api/Board/Tasks/Create?BoardId=${BoardId}`, { data })
            getBoardData()
            reset()
        } catch (error) {
    ...
    return (
        <>
            <div className=' p-2 lg:w-[20vw] sm:w-[40vw]'>
                <form onSubmit={handleSubmit(onSubmit)} method="post">
                    <div className="mb-6">
                        ...
                        />
                        {errors.TaskName && <p className="mt-2 text-sm text-red-600 dark:text-red-500"><span className="font-medium"></span> {errors.TaskName.message}</p>}
                    </div>
                </form>
