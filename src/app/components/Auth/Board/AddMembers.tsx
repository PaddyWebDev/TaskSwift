...
export default function AddMembers({ BoardId, GetMembers }: any) {
    ...
    async function AddMember(MemberName: string) {
        try {
            const response = await axios.post(`/api/Board/AddMember?BoardId=${BoardId}`, { Name: MemberName })
                            ...
                            {loading ? (
                                ...
                                Users && Users ? (Users.map((user: UserType, index: number) => (
                                    <div className='px-5 py-1 flex items-center' key={index}>
                                        <button className='dark:bg-stone-800 bg-stone-200 py-2 px-4 rounded-lg' onClick={() => AddMember(user.name)}>{user.name}</button>
                                    </div>
                                ))) : (<h1> Nothing Found</h1>)
                            }
                        </Suspense>
